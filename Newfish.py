...
def retruescale(x,y,ascale):
    ...
    return ((x-(screen_width/2-zoomx)-screen_width/2) * ascale + screen_width/2),(y-(screen_height/2-zoomy)-screen_height/2) * ascale + screen_height/2
class Food:
...
class Boid:
    ...
    def apply_separation(self, boids, separation_distance):
        ...
        self.velocity[0] -= sum(pos[0] / np.linalg.norm(pos) for pos in positions) * self.separation_resolve
        self.velocity[1] -= sum(pos[1] / np.linalg.norm(pos) for pos in positions) * self.separation_resolve
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_p:
                    paused = not paused
        screen.fill((0,0,0))
        pygame.draw.rect(screen, (0, 100, 255), (*retruescale(0,0,rascale), screen_width*rascale, screen_height*rascale),2)
        ...
        if keys[pygame.K_SPACE]:
            ...
            def mouserpress(strength):
                ...
                for boid in aboid:boid.apply_mouse_repulsion((msx,msy), repulsion_distance/aascale, strength)
