...
class Boid:
    def __init__(self, screen_width, screen_height, 
        ...
        self.velocity[0] *= round(random.choice([-1, 1]), 6)
        self.velocity[1] *= round(random.choice([-1, 1]), 6)
    ...
    def poscalc_angle(self, target,target2):
        return np.atan2(target2[1] - target[1], target2[0] - target[0])
    ...
    def check_bounds(self):       
        ...
        # if not self.posdistance((self.screen_width/2,self.screen_height/2),(self.position[0],self.position[1]))<750-margin:
        #     angle = self.poscalc_angle((self.screen_width/2,self.screen_height/2),(self.position[0],self.position[1]))
        #     self.velocity[1] -= np.sin(angle)*turn_factor
        #     # self.position[0] = min(self.screen_width, max(0, self.position[0]))
        #+     # self.position[1] = min(self.screen_height, max(0, self.position[1]))
        #+ old code top
        #+ new code bottom
        ...
        if self.position[0] < self.barrierx+margin or self.position[0] > self.barrierex-margin or self.position[1] < self.barriery+margin or self.position[1] > self.barrierey-margin and not check_bounde(self.position[0],self.position[1]):
            ...
            for k in range(0, 22):
                ...
                if check_bounde(self.position[0]+np.cos(angle - rj_left) * 32,self.position[1]+np.sin(angle - rj_left) * 32):
                    valid_left_count += 1
                    l_left.append(rj_left)
                if check_bounde(self.position[0]+np.cos(angle + rj_right) * 32,self.position[1]+np.sin(angle + rj_right) * 32):
                    valid_right_count += 1
                    l_right.append(rj_right)
            ...
            if chosen_direction != 0:
                ...
                self.velocity[0] = sp*np.cos(angle)
                self.velocity[1] = sp*np.sin(angle)
    ...
    def draw(self, screen,scale=1):      
        ...
        right_a = np.cos(angle90);left_a = np.sin(angle90)
        ...
        # ])
        pygame.draw.polygon(screen,self.color, [retruescale(self.position[0]-right_a*5,self.position[1]-left_a*5,scale),retruescale(self.position[0]+np.cos(angle)*10,self.position[1]+np.sin(angle)*10,scale),retruescale(self.position[0]+right_a*5,self.position[1]+left_a*5,scale)])
        # pygame.draw.polygon(screen,self.color, [(self.position[0]-right_a*5,self.position[1]-left_a*5),(self.position[0]+np.cos(angle)*10,self.position[1]+np.sin(angle)*10),(self.position[0]+right_a*5,self.position[1]+left_a*5)])
# class Zone:
...
class Slider:
    ...
    def draw(self,scr):
        pygame.draw.rect(scr,(140,140,140),pygame.Rect(self.sl_leftpos-3,self.sl_toppos,self.size[0]+6,self.size[1]))
        pygame.draw.rect(scr,(100,100,100),pygame.Rect(self.sl_leftpos-3,self.sl_toppos,self.size[0]+6,self.size[1]),2)
        pygame.draw.rect(scr,(200,200,200),(self.sl_leftpos,self.pos[1]-5,(self.brect.centerx-self.sl_leftpos),10),0)
        pygame.draw.rect(scr,(255,255,25),self.brect,border_radius=3)
...
def main() -> None:
    ...
    def NewRturnM_scale(x,y,ascale) -> tuple:
        return (x -screen_width/2)/ ascale + screen_width/2,(y -screen_height/2)/ ascale + screen_height/2
    for i in range(96):
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_p:
                    paused = not paused
        screen.fill((0,0,0))
        pygame.draw.circle(screen, (255, 100, 0), (screen_width/2, screen_height/2), 750*aascale,1)
        ...
        if paused:
            for slider in sliders:
                if slider.crect.collidepoint(mouse_x,mouse_y) :
                    ...
                    screen.blit(t, (slider.pos[0]-200-t.get_width(), slider.pos[1]-18))
